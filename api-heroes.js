	...
	//Affichage du héros sélectionné dans le menu et de l'image dans la zone de combat
	showPersos = (persos, team) => {
		...
		if(team == "team team1 close" || team=="team team1"){
  		...
  		persos.forEach(perso => {
  			...
  			if(perso.name.toLowerCase() == persoUser.toLowerCase()){
			    ...
			    // installation d'un écouteur d'évenement qui supprime un superhéros de la liste lors du click sur la petite croix dans le menu 
  	...
  	function AndtheWinnerIs(winnerImage, input1, input2, button1, button2){
		...
		if(winner[0].team == "team2"){
		    ...
		    lifebarG.addEventListener('transitionend', function(e) {
				...
				// lancement de la function avec un délai de 200ms
			  	setTimeout(function(){
					AndtheWinnerIs(WinnerImage, input1, input2, submit1, submit2);
				}, 200);
			});
		}
		else{
...
// Autocomplete -------------------------------------
	/* prend 4 paramètres : l'input, le tableau de héros, 
	l'id du bouton du form et un entier limite */
	function autocomplete(inp, arr, submit, length) {
	  ...
	  inp.addEventListener("input", function(e) {
	      ...
	      /* insère la div de la liste autocomplete entre l'input et le bouton dans le formulaire*/
	      this.parentNode.insertBefore(a, siblingElement);
	      /*for each item in the array...*/
	      for (i = 0; i < arr.length; i++) {
	      	/* le nombre de caractères entré doit être égal à la limite fixé en paramètre 
	...
	autocomplete(document.getElementById("input1"), superheroes, "submit1", 2);
	autocomplete(document.getElementById("input2"), superheroes, "submit2", 2);
